...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    result = [f"Khách sạn tại {city} (ngân sách dưới {format_currency(max_price_per_night)}):"]
    for h in filtered_hotels:
        price_str = format_currency(h['price_per_night'])
        result.append(f"- {h['name']} ({h['stars']} sao) - {h['rating']}⭐ | Khu vực: {h['area']} | Giá: {price_str}/đêm")
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        return "Lỗi: Định dạng chuỗi expenses sai. Vui lòng dùng 'tên_khoản:số_tiền, ...'"
    ...
    for name, amount in expense_dict.items():
        result.append(f"- {name.replace('_', ' ').capitalize()}: {format_currency(amount)}")
    ...
    if remaining < 0:
        result.append(f"Cảnh báo: Vượt ngân sách {format_currency(abs(remaining))}! Cần điều chỉnh.")
